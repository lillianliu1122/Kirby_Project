    ...
    switch (gameState) {
        ...
        case GameState::Playing:
            ...
            for (Enemy* enemy : enemies) {
                ...
                // B. 身體碰撞 (針對 Sparky，只有在 [沒在放電] 時碰到本體才受傷，或者改成碰本體完全不受傷)
                bool isSparky = (dynamic_cast<Sparky*>(enemy) != nullptr);
                if (!isSparky) { // 如果不是 Sparky，才檢查原本的身體碰撞
...
void GameWindow::initStageEnemies(int stage) {
    ...
    if (stage == 1) {
        ...
        enemies.push_back(new Gordo(1000, 550));
        //enemies.push_back(new WaddleDee(1400, 700, 200));
        // frame 2 (x：1621-3240)
        enemies.push_back(new Gordo(2100, 550));
        ...
        // frame 3 (x：3241-4860)
        enemies.push_back(new Gordo(3400, 550));
        enemies.push_back(new Gordo(4000, 550));
        ...
        // Stage 2：加入噴火怪和電電怪
        enemies.push_back(new WaddleDee(400, 800));
